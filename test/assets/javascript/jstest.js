    ...
    $(document).on("click", "#destinationCity", function (event) {
        ...
        $.ajax({
            ...
            for (var i = 0; i < 2; i++) {
                ...
                getRemainingPlaceData(hotelObj.placeId);
            ...
            function getRemainingPlaceData (id2){
                var queryURL3 = "https://maps.googleapis.com/maps/api/place/details/json?placeid=" + id2 + "&key=AIzaSyCx8gqMHIV2OLrVEa-9o5xU34SzJD6W3Js";
                $.ajax({
                    url: queryURL3,
                    method: "GET"
                    ...
                    var url = "https://maps.googleapis.com/maps/api/place/photo?photoreference=" + photoReference2 + "&sensor=false&maxheight=400&maxwidth=400&key=AIzaSyCx8gqMHIV2OLrVEa-9o5xU34SzJD6W3Js";
                    photo2.attr("src", url);
                    hotelObj.link = results3.website;
                    hotelObj.phone = results3.formatted_phone_number;
