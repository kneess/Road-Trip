$(document).ready(function () {
    ...
    $(document).on("click", "#hotSurprise", function (event) {
        ...
        renderResults(random, "hotelResults", 10);
        console.log(random);
    });
    ...
    function getRemainingPlaceData(hotelObj, num) {
        var queryURL3 = "https://maps.googleapis.com/maps/api/place/details/json?placeid=" + hotelObj.placeId + "&key=AIzaSyCNDmPqZOjXBhXXOgsRpnFMEELte5ssmi4";
    ...
    $.ajax({
        ...
        for (var i = 0; i < 2; i++) {
            ...
            restObj.placeId = results[i].place_id;
            restObj.name = results[i].name;
            getRemainingPlaceData(restObj, num);
            // foodResults.push(restaurantObj);
        };
        console.log("restaurant Object", restObj);
    });
...
// var queryURL2 = "https://maps.googleapis.com/maps/api/place/details/json?placeid=" + restObj.placeId + "&key=AIzaSyCNDmPqZOjXBhXXOgsRpnFMEELte5ssmi4";
function getRemainingPlaceData(restObj, num){
var queryURL2 = "https://maps.googleapis.com/maps/api/place/details/json?placeid=" + restObj.placeId + "&key=AIzaSyCx8gqMHIV2OLrVEa-9o5xU34SzJD6W3Js";
$.ajax({
    ...
    restObj.name = results2.name;
    restObj.address = results2.formatted_address;
    restObj.rating = results2.rating;
    restObj.link = results2.website;
    restObj.description = results2.reviews[0].text;
    restObj.contact = results2.formatted_phone_number;
    var photo = $("<img>");
    ...
    restObj.image = "https://maps.googleapis.com/maps/api/place/photo?photoreference=" + photoReference + "&sensor=false&maxheight=400&maxwidth=400&key=AIzaSyCx8gqMHIV2OLrVEa-9o5xU34SzJD6W3Js";

    // photo.attr("src", url);


    foodResults.push(restObj);
    console.log("food results", foodResults);

    renderResults(restObj, "foodResults", num);
    num++;
})
};
});
